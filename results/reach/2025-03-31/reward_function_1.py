...
def custom_reward_function(obs, action, done, env):
    ...
    # Assuming the environment's observed state is composed such that the first three elements
    # represent the gripper position, we can extract it like this:
    gripper_position = obs[:3]

    # Calculate the Euclidean distance between the robotic arm gripper position and the goal
    distance_to_goal = np.linalg.norm(gripper_position - goal)
